...
public class FrozenApocalypseConfig {
    ...
    private void addDefaultApocalypseLevels() {
        this.apocalypseLevels.addAll(List.of(
                ...
                new ApocalypseLevel.Builder(6, 6, 11, 0.4f).freezeEntities(30, 5, 32, 1.0f).grassToFrostedGrass().mobsCanSurviveDaylight().allBiomesSnow().waterToIce().placeSnow().leavesToDeadLeaves().frostedGrassToDeadGrass().iceToPackedIce().lavaToObsidian().placeSnowBlock().build(),
                new ApocalypseLevel.Builder(7, 7, 13, 0.3f).freezeEntities(20, 6, 16, 1.5f).grassToFrostedGrass().mobsCanSurviveDaylight().allBiomesSnow().waterToIce().placeSnow().leavesToDeadLeaves().frostedGrassToDeadGrass().iceToPackedIce().lavaToObsidian().placeSnowBlock().build(),
                new ApocalypseLevel.Builder(8, 8, 15, 0.2f).freezeEntities(20, 7, 16, 2.0f).grassToFrostedGrass().mobsCanSurviveDaylight().allBiomesSnow().waterToIce().placeSnow().leavesToDeadLeaves().frostedGrassToDeadGrass().deadGrassToPermafrost().iceToPackedIce().lavaToObsidian().placeSnowBlock().leavesDecay().build(),
                new ApocalypseLevel.Builder(9, 9, 17, 0.1f).freezeEntities(20, 8, 16, 2.5f).grassToFrostedGrass().mobsCanSurviveDaylight().allBiomesSnow().waterToIce().placeSnow().leavesToDeadLeaves().frostedGrassToDeadGrass().deadGrassToPermafrost().iceToPackedIce().lavaToObsidian().placeSnowBlock().leavesDecay().build()
        ));
    }
