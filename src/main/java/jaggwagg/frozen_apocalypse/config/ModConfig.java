...
public class ModConfig {
    ...
    private void addDefaultApocalypseLevels() {
        this.apocalypseLevels.addAll(List.of(
                ...
                new ApocalypseLevel.Builder(8, 8, 15, 0.2f).freezeEntities(20, 7, 16, 2.0f).grassToFrostedGrass().mobsCanSurviveDaylight().allBiomesSnow().waterToIce().placeSnow().placeIcicles().leavesToDeadLeaves().frostedGrassToDeadGrass().iceToPackedIce().lavaToObsidian().placeSnowBlock().mobsCanSpawnDaylight().deadGrassToPermafrost().leavesDecay().build(),
                new ApocalypseLevel.Builder(9, 9, 17, 0.1f).freezeEntities(20, 8, 16, 2.5f).grassToFrostedGrass().mobsCanSurviveDaylight().allBiomesSnow().waterToIce().placeSnow().placeIcicles().leavesToDeadLeaves().frostedGrassToDeadGrass().iceToPackedIce().lavaToObsidian().placeSnowBlock().mobsCanSpawnDaylight().deadGrassToPermafrost().leavesDecay().build()
        ));
    }
